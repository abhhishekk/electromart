...
<html>
<head>
	<title>Electromart Order Details</title>
  <meta charset="utf-8">
    	...
    	<div align="center" class="mt-5">
    		...
    		<div class="row justify-content-center">
             ...
             <button type="button" data-toggle="modal" data-target="#trackingmodel" class="btn btn-lg btn-primary"><i class="fas fa-biking"></i> Track Your Order</button>&nbsp;&nbsp;
    			    ...
    			    <a href="products.php?order=submitted" class="btn btn-lg btn-dark"><i class="far fa-arrow-alt-circle-left"></i> Go Back</a>
    			</div>
    		</div>
        <br>
         <h6>If you wish to cancel your order, You can <a type="button" data-toggle="modal" data-target="#cancelmodel" style="color: red;">cancel here</a></h6>
        </div>
        	...
        	<thead>
        		...
        		<tbody>
        			...
        			<tr>
        			   ...
        			   <td><?php echo $value['item_pname']; ?></td>
                       <td><img src="uploads/<?php echo $value['item_img']; ?>" height='40px'></td>
        			   <td>&#8377; <?php echo $value['item_pprice']; ?></td>	
        			   <td><?php echo $value['item_qty']; ?></td>
        			   <td>&#8377; <?php echo $value['item_pprice'] * $value['item_qty']; ?></td>
        			 <?php
        			  }
        			 ?>  
      ...
      <div class="modal-body">
         <div class="row">
             <div class="col-md-12">
                ...
                <?php
                 $user = $_SESSION['u_name'];
                 $sql = "SELECT * FROM orders WHERE ordereduser = '$user' ORDER BY orderid DESC LIMIT 1;";
                 ...
                 if ($resultcheck > 0) {
                   while ($row = mysqli_fetch_assoc($result)) {
                      ...
                      //echo "<pre>";
                      //echo $id;
                      $secondsql = "SELECT * FROM orders WHERE orderid='$id';";
     ...
     $sql = "select * from orders where ordereduser='$user' order by orderid desc limit 1;";
       $result = mysqli_query($con, $sql);
        ...
        if ($res_status == 'waiting') {
           ...
           $update = "DELETE FROM orders WHERE orderid = '$orderid';";
           mysqli_query($con, $update);

           echo "<script>alert('Your Product has been cancelled Successfully')</script>";
           echo "<script>window.location='products.php?order=submitted'</script>";
     } else {
       echo "<p class='alert alert-danger'>Sorry, Your Product cannot be cancelled</p>";
     }
   }
   }else {
...
<div class="modal" tabindex="-1" role="dialog" id="cancelmodel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Cancel Your Order Here</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
       <h5>Cancel Your Order Here</h5>       
        <form method="POST">
          <div class="form-group">
            <label>Random Reason<span style="color: red;">*</span></label>
             <select class="form-control" name="select">
               <option>Wrong selection of product</option>
               <option>Wrong quantity of product</option>
