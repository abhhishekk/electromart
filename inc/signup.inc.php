...
if (isset($_POST['submit'])) {
  ...
  if (empty($fname) || empty($uname) || empty($email) || empty($contact) || empty($address) || empty($pwd)) {
    ...
    if ($resultcheck > 0) {
      ...
      $run = "select * from customers where email = '$email';";
      ...
      if ($resultcheck > 0) {
        ...
        $sql = "select contactno from customers where contactno='$contact';";
        $result = mysqli_query($con, $sql);
        $resultcheck = mysqli_num_rows($result);
        ...
        if ($resultcheck > 0) {
          ...
          $insert = "insert into customers ( fullname, username, email, contactno, address, pwd, profile ) 
