<?php
  if (isset($_POST['submit'])) {
    ...
    if (empty($aduname) || empty($adpwd)) {
    	header("Location: ../../../electromart/admin/admin_index.php?login=empty");
  	    exit();
    } else {
    	 ...
    	 if ($resultcheck > 0) {
    	 	while ($row = mysqli_fetch_assoc($result)) {
    	 		...
    	 		$_SESSION['ad_uname'] = $row['ad_uname'];
    	 		 ...
    	 		 header("Location: ../../../electromart/admin/admin_index.php?signup=success");
    	 		 exit();
    	 	}
    	 } else {
    	 	header("Location: ../../../electromart/admin/index.php?login=error");
    	 	exit();
    	 }
    }
