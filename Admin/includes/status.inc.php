...
if (!isset($_SESSION['ad_uname'])) {
 ...
 //Giving enable option to brands
  ...
  if ($resultcheck > 0) {
    ...
    if ($newresult == $stats) {
    	header("Location: ../../../electromart/admin/admin_brand.php?access=alreadygranted");
    	exit();
    } elseif ($newresult != $stats) {
    	$sql = "update brands set status='$stats' where bid='$res';";
    	mysqli_query($con, $sql);

    	header("Location: ../../../electromart/admin/admin_brand.php?access=granted");
    	exit();
    }
  }
